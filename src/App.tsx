...
type Workspace = "hub" | "intel" | "outfit" | "banner";
...
function getDeviceId() {
  const storageKey = "faisal-hub-device";
      ...
      <div className="relative z-10 mx-auto grid min-h-screen max-w-[1440px] lg:grid-cols-[1.15fr_.85fr]">
        <section className="relative flex min-h-[38vh] flex-col justify-between px-6 pb-10 pt-6 sm:px-10 lg:min-h-screen lg:px-16 lg:pb-16 lg:pt-10">
          ...
          >
            ...
            <div>
              <div className="brand-wordmark text-lg font-black tracking-[0.16em]">FAISAL HUB</div>
              <div className="text-[10px] font-bold uppercase tracking-[0.25em] text-white/38">Free Fire Tools</div>
            </div>
          </motion.div>
          ...
          >
            ...
            <p className="mt-6 max-w-md text-base leading-7 text-white/48 sm:text-lg">
              Search Free Fire profiles, inspect match data, render outfits, and generate banner images from one focused hub.
            </p>
          </motion.div>
...
function DataValue({ value }: { value: unknown }) {
  if (value === null || value === undefined || value === "") return <span className="text-white/25">None</span>;
  if (Array.isArray(value)) return <span className="break-words text-orange-100/85">{value.join(", ") || "[]"}</span>;
  if (typeof value === "object") {
    return <pre className="mt-2 max-h-56 overflow-auto whitespace-pre-wrap rounded-xl bg-black/25 p-3 text-[11px] leading-5 text-white/48">{JSON.stringify(value, null, 2)}</pre>;
  }
  if (/time|date|login/i.test(String(value)) && Number.isFinite(Number(value))) return <span>{formatDate(value)}</span>;
...
function DataSection({ title, value }: { title: string; value: unknown }) {
  ...
  return (
    <details className="data-section group" open={title === "AccountInfo" || title === "AccountProfileInfo"}>
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4">
        <span className="font-bold tracking-tight">{formatLabel(title)}</span>
        <span className="text-xs font-bold uppercase tracking-[0.14em] text-orange-300/60 group-open:hidden">Open</span>
        <span className="hidden text-xs font-bold uppercase tracking-[0.14em] text-white/28 group-open:inline">Hide</span>
      </summary>
      <div className="border-t border-white/[0.06] px-5 py-4">
        {entries.length ? (
          <div className="grid gap-3 sm:grid-cols-2">
            {entries.map(([key, item]) => (
              <div key={key} className="rounded-xl border border-white/[0.06] bg-white/[0.018] p-3">
                <div className="mb-1 text-[10px] font-bold uppercase tracking-[0.16em] text-white/28">{formatLabel(key)}</div>
                <div className="text-sm leading-6 text-white/72"><DataValue value={item} /></div>
              </div>
            ))}
          </div>
        ) : (
...
function FullDataExplorer({ payload, banStatus }: { payload: unknown; banStatus?: unknown }) {
  const root = asRecord(payload);
  const sections = Object.entries(root).filter(([key]) => key !== "credits");
  const completePayload = banStatus ? { ...root, BanCheckInfo: banStatus } : root;

  return (
    <div className="border-t border-white/[0.07] px-4 py-6 sm:px-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          <div className="section-kicker">Complete player data</div>
          <h4 className="mt-2 text-xl font-black tracking-[-0.03em]">All information returned by API</h4>
        </div>
        <span className="rounded-full border border-emerald-300/20 bg-emerald-300/[0.06] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-200/70">
          Full info
        </span>
      </div>
      <div className="space-y-3">
        {sections.map(([key, value]) => <DataSection key={key} title={key} value={value} />)}
        {banStatus ? <DataSection title="BanCheckInfo" value={banStatus} /> : null}
        <details className="data-section group">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4">
            <span className="font-bold tracking-tight">Raw JSON backup</span>
            <span className="text-xs font-bold uppercase tracking-[0.14em] text-orange-300/60 group-open:hidden">Open</span>
            <span className="hidden text-xs font-bold uppercase tracking-[0.14em] text-white/28 group-open:inline">Hide</span>
          </summary>
          <pre className="max-h-[520px] overflow-auto border-t border-white/[0.06] bg-black/25 p-5 text-xs leading-6 text-white/50">{JSON.stringify(completePayload, null, 2)}</pre>
        </details>
      </div>
    </div>
  );
...
function InfoView({ result }: { result: IntelResult }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
      ...
      <FullDataExplorer payload={result.payload} banStatus={result.banStatus} />
    </motion.div>
  );
}
...
function PlayerIntel({ onOpenOutfit, onOpenBanner }: { onOpenOutfit: (uid: string) => void; onOpenBanner: (uid: string) => void }) {
        ...
        {result?.uid && result.mode === "info" && (
          <div className="flex flex-wrap gap-2">
            <button onClick={() => onOpenOutfit(result.uid!)} className="secondary-button flex items-center gap-2 px-4 py-3 text-sm font-bold">
              <Shirt className="h-4 w-4" /> View outfit
            </button>
            <button onClick={() => onOpenBanner(result.uid!)} className="secondary-button flex items-center gap-2 px-4 py-3 text-sm font-bold">
              <ImageIcon className="h-4 w-4" /> View banner
            </button>
          </div>
        )}
      </div>
...
function BannerStudio({ initialUid = "" }: { initialUid?: string }) {
  const [uid, setUid] = useState(initialUid);
  const [imageUrl, setImageUrl] = useState("");
  const [activeUid, setActiveUid] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  function loadBanner(event: FormEvent) {
    event.preventDefault();
    const cleaned = uid.trim();
    if (!/^\d{5,15}$/.test(cleaned)) {
      setError("Enter a valid numeric player UID.");
    ...
    setLoading(true);
    setActiveUid(cleaned);
    setImageUrl(`${createFfUrl("/banner/profile", { uid: cleaned })}&_=${Date.now()}`);
  }
  ...
  return (
    ...
    >
      <div className="mb-8">
        <div className="section-kicker">Banner image</div>
        <h1 className="mt-2 text-4xl font-black tracking-[-0.05em] sm:text-5xl">Generate the profile banner.</h1>
        <p className="mt-3 max-w-xl text-sm leading-6 text-white/42 sm:text-base">Create a banner image with avatar, player name, guild style, and level where the API provides it.</p>
      </div>

      <div className="grid gap-5 lg:grid-cols-[340px_1fr]">
        <div className="workspace-panel h-fit p-5 sm:p-6">
          <div className="mb-7 flex h-12 w-12 items-center justify-center rounded-2xl border border-fuchsia-300/15 bg-fuchsia-400/[0.08] text-fuchsia-300">
            <ImageIcon className="h-5 w-5" />
          </div>
          <h2 className="text-xl font-bold tracking-tight">Banner request</h2>
          <p className="mt-2 text-sm leading-6 text-white/38">Enter a UID and generate the public player banner image.</p>

          <form onSubmit={loadBanner} className="mt-7 space-y-4">
            <label className="field-label block">
              <span>Player UID</span>
              <div className="field-control field-control-light mt-2">
                <UserRound className="h-[18px] w-[18px] text-white/28" />
            ...
            {error && <div className="flex gap-2 text-sm text-red-300"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" /> {error}</div>}
            <motion.button whileTap={{ scale: 0.98 }} disabled={loading} className="primary-button flex w-full items-center justify-center gap-2 px-5 py-4 text-sm font-extrabold uppercase tracking-[0.11em] disabled:opacity-60">
              {loading ? <LoaderCircle className="h-5 w-5 animate-spin" /> : <ImageIcon className="h-4 w-4" />}
              {loading ? "Generating" : "Show banner"}
            </motion.button>
          </form>

          <div className="mt-6 border-t border-white/[0.07] pt-5 text-xs leading-5 text-white/30">
            Banner rendering is served directly by the Free Fire multipurpose API.
          </div>
        </div>
        ...
        <div className="outfit-stage workspace-panel relative min-h-[520px] overflow-hidden">
          ...
          <AnimatePresence mode="wait">
            {!imageUrl ? (
              <motion.div key="banner-empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative flex min-h-[520px] flex-col items-center justify-center px-7 text-center">
                <motion.div className="relative flex h-28 w-28 items-center justify-center" animate={{ y: [0, -7, 0] }} transition={{ duration: 3.4, repeat: Infinity, ease: "easeInOut" }}>
                  <div className="absolute inset-0 rounded-full border border-dashed border-fuchsia-300/20" />
                  <div className="absolute inset-5 rounded-full border border-white/[0.07]" />
                  <ImageIcon className="h-10 w-10 text-fuchsia-300/75" />
                </motion.div>
                <h3 className="mt-7 text-2xl font-bold tracking-tight">Your banner will appear here</h3>
                <p className="mt-2 max-w-md text-sm leading-6 text-white/35">Enter a UID to render the player banner.</p>
              </motion.div>
            ) : (
              <motion.div key={imageUrl} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative flex min-h-[520px] flex-col">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.07] px-5 py-4 sm:px-6">
                  <div>
                    <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/30">Rendered banner</div>
                    <div className="mt-1 font-bold">UID {activeUid}</div>
                  </div>
                  {!loading && (
                    <div className="flex gap-2">
                ...
                <div className="relative flex flex-1 items-center justify-center p-4 sm:p-7">
                  {loading && (
                    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-[#111316]/90 backdrop-blur-sm">
                      <div className="scan-loader relative flex h-20 w-20 items-center justify-center rounded-full"><ImageIcon className="h-6 w-6 text-orange-300" /></div>
                      <span className="mt-5 text-xs font-bold uppercase tracking-[0.18em] text-white/35">Building banner image</span>
                    </div>
                  )}
                  <motion.img
                    src={imageUrl}
                    alt={`Profile banner for player ${activeUid}`}
                    className="max-h-[680px] w-full max-w-[900px] object-contain drop-shadow-[0_30px_60px_rgba(0,0,0,.48)]"
                    onLoad={() => setLoading(false)}
                    onError={() => {
                      setLoading(false);
                      setImageUrl("");
                      setError("The banner image could not be generated. Check the UID, then try again.");
                    }}
                    initial={{ opacity: 0, scale: 0.97 }}
                    animate={{ opacity: loading ? 0 : 1, scale: loading ? 0.97 : 1 }}
                    transition={{ duration: 0.5 }}
...
function HubCard({ title, description, icon: Icon, accent, onClick }: { title: string; description: string; icon: LucideIcon; accent: string; onClick: () => void }) {
  return (
    <motion.button
      onClick={onClick}
      className={`hub-card group relative min-h-[245px] overflow-hidden rounded-[22px] border border-white/[0.09] bg-[#12131a]/90 p-6 text-left ${accent}`}
      whileHover={{ y: -7, scale: 1.01 }}
      whileTap={{ scale: 0.985 }}
      transition={{ type: "spring", stiffness: 260, damping: 24 }}
    >
      <div className="hub-card-glow absolute inset-0 opacity-0 transition group-hover:opacity-100" />
      <div className="relative z-10 flex h-full flex-col">
        <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl border border-current/25 bg-current/10">
          <Icon className="h-7 w-7" />
        </div>
        <h3 className="text-2xl font-black tracking-[-0.04em] text-white">{title}</h3>
        <p className="mt-3 max-w-sm text-sm leading-6 text-white/45">{description}</p>
        <div className="mt-auto flex items-end justify-between pt-8">
          <span className="rounded-full border border-emerald-300/20 bg-emerald-300/[0.08] px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-emerald-300">Live</span>
          <span className="flex h-9 w-9 items-center justify-center rounded-full border border-current/30 bg-current/10 transition group-hover:translate-x-1">
...
function HubHome({ chooseWorkspace }: { chooseWorkspace: (next: Workspace) => void }) {
  return (
    <motion.section
      key="hub"
      initial={{ opacity: 0, y: 12 }}
    ...
    >
      <div className="mb-8">
        <div className="section-kicker">Faisal Hub</div>
        <h1 className="mt-2 text-4xl font-black tracking-[-0.05em] sm:text-6xl">Choose your tool.</h1>
        <p className="mt-4 max-w-2xl text-sm leading-6 text-white/42 sm:text-base">Two main tools are ready: player info/stats/search and outfit image. Banner image has also been added as a third visual tool.</p>
      </div>

      <div className="hub-grid grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        <HubCard
          title="Player Info/stats/search"
          description="Full account information, ban check, BR/CS stats, guild data, equipped IDs, social info, and nickname search."
          icon={BarChart3}
          accent="text-violet-300"
          onClick={() => chooseWorkspace("intel")}
        />
        <HubCard
          title="Outfit Image"
          description="Render the player's equipped outfit items on a clean visual card from a Free Fire UID."
          icon={Shirt}
          accent="text-cyan-300"
          onClick={() => chooseWorkspace("outfit")}
        />
        <HubCard
          title="Banner Image"
          description="Generate a profile banner image with avatar, player name, level, and profile styling."
          icon={ImageIcon}
          accent="text-fuchsia-300"
          onClick={() => chooseWorkspace("banner")}
        />
      </div>
    </motion.section>
  );
...
function Dashboard({ user, sessionid, onLogout }: { user: AuthUser; sessionid: string; onLogout: () => void }) {
  ...
  function openBanner(uid: string) {
    setBannerUid(uid);
    setWorkspace("banner");
  }
    ...
    <main className="dashboard-shell min-h-screen bg-[#0d0f11] text-white">
      ...
      <header className="fixed inset-x-0 top-0 z-40 flex h-20 items-center border-b border-white/[0.07] bg-[#0d0f11]/85 px-5 backdrop-blur-xl lg:left-64 lg:px-8">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 truncate text-sm font-bold">
            {workspace === "hub" && "Tool launcher"}
            {workspace === "intel" && "Player Info/stats/search"}
            {workspace === "outfit" && "Outfit Image"}
            {workspace === "banner" && "Banner Image"}
          </div>
        </div>
      ...
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-64 flex-col border-r border-white/[0.07] bg-[#101214] px-4 pb-5 pt-6 lg:flex">
        <div className="flex items-center gap-3 px-2">
          <BrandMark className="h-10 w-10" />
          <div>
            <div className="brand-wordmark text-base font-black tracking-[0.15em]">FAISAL HUB</div>
            <div className="text-[9px] font-bold uppercase tracking-[0.25em] text-white/30">Free Fire Tools</div>
          </div>
        </div>

        <nav className="mt-12 space-y-2">
          <button onClick={() => chooseWorkspace("hub")} className={`nav-item ${workspace === "hub" ? "nav-item-active" : ""}`}>
          ...
          <button onClick={() => chooseWorkspace("banner")} className={`nav-item ${workspace === "banner" ? "nav-item-active" : ""}`}>
            <ImageIcon className="h-5 w-5" />
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <BrandMark className="h-10 w-10" />
                  <div className="brand-wordmark text-sm font-black tracking-[0.14em]">FAISAL HUB</div>
                </div>
              ...
              <nav className="mt-10 space-y-2">
                <button onClick={() => chooseWorkspace("hub")} className={`nav-item ${workspace === "hub" ? "nav-item-active" : ""}`}><Sparkles className="h-5 w-5" /> Tool hub</button>
                <button onClick={() => chooseWorkspace("intel")} className={`nav-item ${workspace === "intel" ? "nav-item-active" : ""}`}><Search className="h-5 w-5" /> Info/stats/search</button>
                ...
                <button onClick={() => chooseWorkspace("banner")} className={`nav-item ${workspace === "banner" ? "nav-item-active" : ""}`}><ImageIcon className="h-5 w-5" /> Banner image</button>
              </nav>
      ...
      <div className="relative z-10 px-4 pb-12 pt-28 sm:px-7 lg:ml-64 lg:px-9 lg:pt-28">
        <AnimatePresence mode="wait">
          {workspace === "hub" && <HubHome chooseWorkspace={chooseWorkspace} />}
          {workspace === "intel" && <PlayerIntel onOpenOutfit={openOutfit} onOpenBanner={openBanner} />}
          {workspace === "outfit" && <OutfitStudio initialUid={outfitUid} />}
          {workspace === "banner" && <BannerStudio initialUid={bannerUid} />}
        </AnimatePresence>
      </div>
    </main>
  );
